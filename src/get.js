function init(data){
  ...
  var contract = data.config.tokenContract
  var powercontract = data.config.powerContract
  ...
  async function balance (account) {
    console.log('boidjs get balance:',account,contract)
    try {
  ...
  async function accountDevices (account, protocol) {
    try {
      const res = await rpc.get_table_rows({
        json: true,
        code: powercontract,
      ...
      console.error(error)
      await sleep(1000)
      return accountDevices(account, protocol)
    }
  }
